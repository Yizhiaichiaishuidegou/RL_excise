...
class Solve:
    ...
    def mc_basic(self, length=30, epochs=10):
        ...
        for epoch in range(epochs):
            for state in range(self.state_space_size):
                for action in range(self.action_space_size):
                    episode = self.obtain_episode(self.policy, state, action, length) # for every (s,a) product  one episode
                    g = 0
                ...
                for action in range(self.action_space_size):
                    ...
                    for step in range(len(episode) - 1, -1, -1):
                        ...
                        if [state, action] not in visit_list:
                            ...
                            print(np.array(returns[state][action]).mean())
    ...
    def mc_epsilon_greedy(self, length=1000, epsilon=1, tolerance=1):
        ...
        while True:
            ...
            # if continue three times the q_value tolerance ,we think the policy converge
            if len(norm_list) >= 3:
        ...
        while num_episodes > 0:
            ...
            while not done:
                ...
                action = np.random.choice(np.arange(self.action_space_size),
                                          p=self.policy[state])
                _, reward, done, _, _ = self.env.step(action) #  when angent arriver target state the trajectory will stop ,that done = true
                episode_length += 1
                ...
                # this sarsa core q_value_k+1 = q_value_k - alpha *(q_value_k - (r+gamma*qvalue[next_state, next_action]))
    ...
    def expected_sarsa(self, alpha=0.1, epsilon=1, num_episodes=1000):
        ...
        while num_episodes > 0:
            ...
            while not done:
                ...
                episode_length += 1
                total_rewards += reward
                # expected_sarsa this make expected state value no required a_t+1
